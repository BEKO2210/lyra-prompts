---
id: "#3143"
titel: "High-Fidelity Portrait of a Young Blonde Woman"
kategorie: "Bildbearbeitung & Visualisierung"
unterkategorie: "Importiert"
tags: ["high", "fidelity", "portrait", "young", "blonde"]
plattformen: ["ChatGPT", "Claude", "Gemini"]
schwierigkeit: "Anfänger"
quelle: "awesome-chatgpt-prompts"
autor: "rhieskhiez@gmail.com,rhieskhiez.creative@gmail.com"
erstellt: "2026-03-09"
---

## Prompt

```
{
  "prompt": {
    "subject": {
      "type": "close-up portrait",
      "person": {
        "description": "young woman",
        "hair": {
          "color": "blonde",
          "length": "long",
          "style": "high ponytail"
        },
        "head_covering": "none",
        "physique": "curvy, natural body type",
        "expression": "warm, friendly smile",
        "pose": "turned slightly to the side, looking at camera"
      },
      "clothing": {
        "garment": "white spaghetti strap tank top",
        "fabric_specifications": {
          "weight": "190 GSM",
          "composition": "82% Polyester, 18% Spandex",
          "features": "4-way stretch"
        }
      }
    },
    "background": {
      "element": "textured teal wall"
    },
    "lighting": {
      "type": "soft, natural lighting",
      "effect": "gently illuminates facial features and shoulders",
      "quality": "warm, flattering"
    },
    "mood": {
      "attributes": ["lively", "casual", "confident", "welcoming", "comfortable", "everyday elegance"]
    },
    "composition": {
      "shot_type": "close-up (CU)",
      "orientation": "vertical",
      "aspect_ratio": "9:16"
    },
    "style": {
      "tone": "respectful and tasteful",
      "content_rating": "appropriate, non-provocative, non-sexual"
    }
  },
  "technical_parameters": {
    "model": "gpt-image-1.5-high-fidelity",
    "aspect_ratio": "9:16",
    "quality": "high-fidelity"
  }
}
```

## Anwendung

**Thema: Young Woman, High Ponytail** — Generiert beeindruckende KI-Bilder mit optimierten Beschreibungen. Kopiere den Prompt in ChatGPT (DALL-E), Midjourney oder andere Bild-KIs.

Kopiere den Prompt und fuege ihn in ChatGPT, Claude oder Gemini ein.
Passe die Details an deine Beduerfnisse an.

## Variationen

- Aendere Farben, Stimmung oder Beleuchtung nach Wunsch
- Fuege "--ar 16:9" (Midjourney) oder Formatangaben hinzu
- Ersetze das Hauptmotiv durch dein eigenes Thema
- Kombiniere verschiedene Stile (z.B. "watercolor meets cyberpunk")
